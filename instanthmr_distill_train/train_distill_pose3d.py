#!/usr/bin/env python3
# ============================================================
# Keypoint + camera student — NO MHR mesh model / params.
# ------------------------------------------------------------
# A stripped-down sibling of train_distill.py. It predicts and
# supervises the 70 native 2D + 3D keypoints AND the camera
# translation (cam_trans) — which is NOT an MHR parameter and is
# required by the demo (joints_3d_cam = joints_3d + cam_trans).
#
# Removed vs train_distill.py: the MHR body rig (mhr_model.pt),
# the mhr_params / shape_params heads, and every MHR-derived loss
# (mhr_joints / structural / feet / reproj_mhr / mesh metrics) —
# i.e. exactly the terms that were causing the periodic loss spikes.
#
# Model out:  cam_trans [B,3], joints_2d [B,70,2] ([-1,1] crop),
#             joints_3d [B,70,3] (root-centred metres).
# Loss:       SmoothL1(2D) + SmoothL1(3D) + MSE(cam) + L1(reproj).
# Metrics:    MPJPE / PA-MPJPE on the 3D head.
# Export:     5 ONNX outputs in demo order (mhr_params & shape_params
#             zero-filled) so it works with the existing demo.py.
#
# Sanity:  python3 train_distill_pose3d.py --overfit-test --data_root ../data
# Full:    python3 train_distill_pose3d.py --data_root /datasets/instanthmr_data --num_workers 8
# ============================================================
import os
...
class PoseConfig:
    ...
    w_cam: float = 0.1           # direct MSE supervision on cam_trans (stable; no 1/z reprojection)
...
class SAM3DStudentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "joints_2d": joints_2d,                                               # [70,2] crop [-1,1]
            "joints_3d": torch.from_numpy(ann["joints_3d"]).float(),              # [70,3] root-centred
            "cam_trans": torch.from_numpy(ann["cam_trans"]).float(),             # [3]  camera translation
        }


def build_dataloaders(cfg):
    full_dataset = SAM3DStudentDataset(cfg.data_root, augment=cfg.augment,
                                       max_images=cfg.max_images, per_dataset_caps=cfg.per_dataset_caps)
...
class InstantHMRPoseStudent(nn.Module):
    ...
    def forward(self, images, cliff_cond):
        ...
        pred_2d = self.head_2d_xy(self.head_2d_feat(feat_2d))
        pred_3d = self.head_3d(feat_3d)
        return {"cam_trans": pred_cam, "joints_2d": pred_2d, "joints_3d": pred_3d}
...
class Pose3DLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        # Direct camera supervision only — SmoothL1 (bounded gradient, robust to large
        # depth targets); MSE here diverges at the overfit LR, no 1/z reprojection term.
        losses['loss_cam'] = self.l1(preds["cam_trans"], targets["cam_trans"]) * self.cfg.w_cam
        losses['total_loss'] = losses['loss_2d'] + losses['loss_3d'] + losses['loss_cam']
        return losses
...
def run_overfit_test(cfg, train_loader, steps=800, subset=8, lr=1e-3):
    ...
    for step in range(steps):
        ...
        if step % 50 == 0 or step == steps - 1:
            ...
            model.train()
            print(f"  step {step:04d} | loss {loss.item():.5f} | 2d {losses['loss_2d'].item():.4f} "
                  f"| 3d {losses['loss_3d'].item():.4f} | cam {losses['loss_cam'].item():.4f} "
                  f"| MPJPE {m['MPJPE']:.1f} mm | PA {m['PA_MPJPE']:.1f} mm")

    ok = last_loss < first_loss * 0.05
    print(f"\n{'✅ SUCCESS' if ok else '❌ FAIL'}: loss {first_loss:.4f} -> {last_loss:.5f} "
          f"({'overfit, gradients flow' if ok else 'did NOT drop enough — check the setup'})")
    return ok
